...
export const I18N: Record<Lang, Dict> = {
  ko: {
    ...
    resultTitle: '관계의 궁합',
    compatScore: '궁합 점수',
    compatLabel: (s) => s >= 85 ? '깊은 인연' : s >= 70 ? '잘 맞는 궁합' : s >= 55 ? '서로 물들어 가는 궁합' : s >= 45 ? '서로를 알아가는 궁합' : s >= 30 ? '천천히 맞춰가는 궁합' : '다름을 배워가는 궁합',
    synergies: '서로를 채우는 궁합',
    conflicts: '조심스러운 궁합',
  ...
  en: {
    ...
    compatLabel: (s) => s >= 85 ? 'Deep kinship' : s >= 70 ? 'Aligned grain' : s >= 55 ? 'Growing closer' : s >= 45 ? 'Still discovering each other' : s >= 30 ? 'Attuning over time' : 'Learning through difference',
  ...
  ja: {
    ...
    resultTitle: '関係の機微',
    compatScore: '相性スコア',
    compatLabel: (s) => s >= 85 ? '深いご縁' : s >= 70 ? 'よく合う機微' : s >= 55 ? '少しずつ近づく機微' : s >= 45 ? '互いを知っていく機微' : s >= 30 ? '少しずつ調える機微' : '違いから学ぶ機微',
    synergies: '互いを満たす機微',
    conflicts: '慎重になりたい機微',
  ...
  zh: {
    ...
    resultTitle: '关系的纹理',
    compatScore: '契合度',
    compatLabel: (s) => s >= 85 ? '深厚的缘分' : s >= 70 ? '契合的纹理' : s >= 55 ? '渐渐相融的纹理' : s >= 45 ? '彼此正在了解' : s >= 30 ? '慢慢磨合的纹理' : '在差异中学习',
    synergies: '彼此成全的纹理',
    conflicts: '需要留心的纹理',
  ...
  es: {
    ...
    compatLabel: (s) => s >= 85 ? 'Vínculo profundo' : s >= 70 ? 'Textura afín' : s >= 55 ? 'Cada vez más cerca' : s >= 45 ? 'Aún se están descubriendo' : s >= 30 ? 'Sintonizando con el tiempo' : 'Aprendiendo en la diferencia',
