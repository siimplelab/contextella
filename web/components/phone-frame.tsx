...
export function PhoneFrame({ children, dark = true }: { children: React.ReactNode; dark?: boolean }) {
  // Desktop scale-to-fit so the 402×874 frame is fully visible
  ...
  useEffect(() => {
    const fit = () => {
      ...
      setScale(Math.min((w - padW) / DEV_W, (h - padH) / DEV_H, 1));
    };
    fit();
  ...
  return (
    <div style={{
      ...
      <div style={{ transform: `scale(${scale})`, transformOrigin: 'center center' }}>
        <div style={{
          width: DEV_W, height: DEV_H, borderRadius: 48, overflow: 'hidden',
