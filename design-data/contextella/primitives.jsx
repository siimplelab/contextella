// primitives.jsx — Contextella visual primitives
// ElementOrb, Constellation, OrbitalViz, GridViz, CompatGauge, StarField

// ────────────────────────────────────────────────────────────
// Animated element orb — soft gradient sphere
// ────────────────────────────────────────────────────────────
function ElementOrb({ element = 'water', size = 120, animated = true }) {
    ...
    <div style={{
      ...
      <svg width={size} height={size} viewBox="0 0 120 120" style={{ overflow: 'visible' }}>
        ...
        <circle cx="60" cy="60" r="58" fill={`url(#${id}-glow)`}>
          {animated && (
            <animate attributeName="r" values="56;60;56" dur="6s" repeatCount="indefinite" />
          )}
        </circle>
        {/* core sphere */}
        ...
        <ellipse cx="60" cy="58" rx="40" ry="12" fill={e.c3} opacity="0.22" filter={`url(#${id}-blur)`}>
          {animated && (
            <animateTransform attributeName="transform" type="rotate"
              from="360 60 60" to="0 60 60" dur="30s" repeatCount="indefinite" />
          )}
        </ellipse>
        {/* highlight */}
...
// ────────────────────────────────────────────────────────────
function StarField({ count = 60, seed = 1, opacity = 0.85 }) {
...
// ────────────────────────────────────────────────────────────
// Constellation visualizer — node graph w/ star lines
// ────────────────────────────────────────────────────────────
function ConstellationViz({ network, width = 360, height = 320, accent = '#E8D4A2',
                            onSelect, selectedId }) {
  ...
  return (
    <div style={{ position: 'relative', width, height }}>
      ...
      <svg width={width} height={height} style={{ position: 'absolute', inset: 0 }}>
        ...
        {[0.45, 0.75, 1].map((f, i) => (
          <circle key={i} cx={cx} cy={cy} r={maxR * f}
                  fill="none" stroke={accent} strokeOpacity={0.1}
                  strokeDasharray="2 4" />
        ))}
        {/* central aura */}
        ...
        {others.map((p) => {
          ...
          const opacity = 0.25 + (p.score / 100) * 0.55;
          return (
            <line key={`l-${p.id}`}
                  x1={cx} y1={cy} x2={pos.x} y2={pos.y}
                  stroke={accent} strokeWidth={0.8 + (p.score / 100) * 1.2}
                  strokeOpacity={opacity} />
          );
        })}

        {/* small star sparkles along lines */}
        {others.map((p) => {
          const pos = positions[p.id];
          const t = 0.42;
          return (
      ...
      <PersonNode person={me} x={positions.me.x} y={positions.me.y} size={64}
                  isMe selected={selectedId === 'me'} onClick={() => onSelect && onSelect('me')} />
      {/* other nodes */}
      {others.map((p) => (
...
// ────────────────────────────────────────────────────────────
// Orbital visualizer — concentric rings with avatars
// ────────────────────────────────────────────────────────────
function OrbitalViz({ network, width = 360, height = 320, accent = '#E8D4A2',
                     onSelect, selectedId }) {
  const cx = width / 2, cy = height / 2;
  const me = network.find(p => p.id === 'me');
  const others = network.filter(p => p.id !== 'me');
  const maxR = Math.min(width, height) * 0.42;
  // group by score band → ring index
  const ring = (score) => score >= 80 ? 0 : score >= 65 ? 1 : 2;
  const ringR = [maxR * 0.42, maxR * 0.72, maxR * 1.0];

  // assign equal-spaced angles per ring
  const positions = React.useMemo(() => {
    const buckets = [[], [], []];
    others.forEach(p => buckets[ring(p.score)].push(p));
    const map = { me: { x: cx, y: cy } };
      ...
      <svg width={width} height={height} style={{ position: 'absolute', inset: 0 }}>
        ...
        {ringR.map((r, i) => (
          <circle key={i} cx={cx} cy={cy} r={r}
                  fill="none" stroke={accent} strokeOpacity={0.18 - i * 0.04}
                  strokeWidth={1} />
        ))}
        <circle cx={cx} cy={cy} r={62} fill="url(#orb-aura)" />
      </svg>
      <PersonNode person={me} x={cx} y={cy} size={64} isMe
                  selected={selectedId === 'me'} onClick={() => onSelect && onSelect('me')} />
      {others.map(p => (
        <PersonNode key={p.id} person={p} x={positions[p.id].x} y={positions[p.id].y}
...
// ────────────────────────────────────────────────────────────
// Card grid visualizer — simpler list-card view
// ────────────────────────────────────────────────────────────
function GridViz({ network, accent = '#E8D4A2', onSelect, selectedId, lang = 'ko' }) {
  const others = network.filter(p => p.id !== 'me');
  return (
    <div style={{
...
// ────────────────────────────────────────────────────────────
// PersonNode — used inside constellation/orbital
// ────────────────────────────────────────────────────────────
function PersonNode({ person, x, y, size, isMe, selected, onClick }) {
...
// ────────────────────────────────────────────────────────────
// Compatibility gauge — circular arc
// ────────────────────────────────────────────────────────────
function CompatGauge({ score = 75, size = 200, accent = '#E8D4A2', label = '' }) {
  const stroke = 10;
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size }}>
      ...
      <div style={{
        ...
        {label && (
          <div style={{
            fontSize: 13, color: accent, marginTop: 6,
...
// ────────────────────────────────────────────────────────────
// Compat star rating (alternate viz)
// ────────────────────────────────────────────────────────────
function CompatStars({ score = 75, accent = '#E8D4A2', label = '', size = 200 }) {
  const filled = score / 20; // 0-5
  return (
    <div style={{
      ...
      {label && (
        <div style={{ fontSize: 13, color: accent, letterSpacing: -0.2, fontWeight: 500 }}>{label}</div>
      )}
    </div>
  );
}
...
// ────────────────────────────────────────────────────────────
// Compat — two merging orbs
// ────────────────────────────────────────────────────────────
function CompatMerge({ score = 75, accent = '#E8D4A2', label = '', size = 200,
...
Object.assign(window, {
  ElementOrb, StarField, ConstellationViz, OrbitalViz, GridViz,
  PersonNode, CompatGauge, CompatStars, CompatMerge,
});
